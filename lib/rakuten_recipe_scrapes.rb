module RakutenRecipeScrapes
  ...
  def scrape_rakuten_recipes(url)
    ...
    doc.xpath("#{html_path}div[3]/section/ul").css('li').map do |node|
      register_ingredients_from_html(node)
    end
  end
  ...
  def register_recipe_from_html(doc, html_path)
    ...
    @recipe = Recipe.find_or_initialize_by(recipe_url: params[:url], cuisine_name: cuisine_name, originator: originator)
    if @recipe.new_record? && !@recipe.save
        render :about
    end
  end

  def register_ingredients_from_html(node)
